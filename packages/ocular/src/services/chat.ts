...
class ChatService extends TransactionBaseService {
  ...
  async* chatWithStreaming(chatId: string, message: string, context: ChatContext): AsyncGenerator<ChatResponseChunk, void> {
    ...
    const assistantMessage = messageRepository.create({ chat_id: chatId, user_id: this.loggedInUser_.id, content: this.removeQuestions(chatResponseContent), role: MessageRoles.ASSISTANT })
    const savedAssistantMessage = await messageRepository.save(assistantMessage)
  ...
  removeQuestions(text: string): string {
    // Regular expression to match the text starting with << and ending with >>
    const regex = /<<.*?>>/g;
